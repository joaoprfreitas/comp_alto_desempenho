    ...
    for (int i = 0; i < n_processes; i++) {
        ...
        if(myrank == i){
           // printf("My rank %d faz broadcast\n", i);
            ...
            memcpy(x_recebido, x, NODE_SIZE * sizeof(int));
            memcpy(y_recebido, y, NODE_SIZE * sizeof(int));
            memcpy(z_recebido, z, NODE_SIZE * sizeof(int));
        }else{
       //     printf("My rank %d, tenta receber de %d\n", myrank, i);
